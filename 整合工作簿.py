...
def integrate_sheets(bi_df, adi_df):
    ...
    adi = _keep_max_per_key(adi_df, 'ADI')
...
def main(path):
    ...
    # 输入校验：Sheet1 需含 BI*、Sheet2 需含 ADI
    if 'BI*' not in bi.columns or 'ADI' not in adi.columns:
        raise SystemExit('Sheet1/Sheet2 列不匹配：Sheet1 需含 BI*，Sheet2 需含 ADI。')
